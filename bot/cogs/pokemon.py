...
class Miscellaneous(Cog):
    ...
    async def pokemon(self, ctx, pokemon = None):
        ...
        pokeEmbed.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        ...
        pokeEmbed.add_field(name="Types", value=", ".join([type.type.name.title() for type in data.types]))
        pokeEmbed.add_field(name="Abilities", value=", ".join([ability.ability.name.title() for ability in data.abilities]))
        try:
            evolution = client.get_pokemon_species(pokemon)[0].evolves_from_species.name
